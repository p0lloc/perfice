<script lang="ts">
    ...
    import ComparisonConditionRenderer from "@perfice/components/goal/editor/condition/comparison/ComparisonConditionRenderer.svelte";
    import GoalMetConditionRenderer from "@perfice/components/goal/editor/condition/goalMet/GoalMetConditionRenderer.svelte";
    ...
    let name = $derived(
        GOAL_CONDITION_TYPES.find((t) => t.type == condition.type)?.name ??
            "Unknown",
    );
</script>

<div class="card p-4">
    <div class="row-between text-gray-500 dark:text-gray-200 mb-2">
        {name}

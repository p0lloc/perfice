    ...
    <div class="card p-4 rounded-xl mt-4 flex flex-col gap-4">
        {#if import.meta.env.DEV}
            <div class="mt-8 flex flex-col gap-4">
                ...
                <Button onClick={() => sync.fullPull(false)}
                    >Check mismatch</Button
                >
                <Button
                    onClick={async () =>
                        alert(
                            Object.entries(await sync.calculateChecksums())
                                .map(([k, v]) => `${k}: ${v.substring(0, 5)}`)
                                .join(" "),
                        )}
                >
                    Calculate checksums
...
<style>
    ...
    :global(.settings-container) {
        @apply card p-4 mt-4 flex flex-col gap-4;
    }
</style>

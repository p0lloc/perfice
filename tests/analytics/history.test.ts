...
test("saves history successfully", async () => {
    ...
    let [rawValues] = await analytics.fetchRawValues(SimpleTimeScopeType.DAILY, date, 7)
...
test("timestamp remains same", async () => {
    ...
    let [rawValues1] = await analytics.fetchRawValues(SimpleTimeScopeType.DAILY, date1, 7)
    ...
    let [rawValues2] = await analytics.fetchRawValues(SimpleTimeScopeType.DAILY, date2, 8)

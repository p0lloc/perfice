<script lang="ts" generics="T">
    ...
    function onNodeValueChange(
        ...
        onChange(
            updateIndexInArray(value, index, {
    ...
    let questionDropdown = $derived(
        availableDynamic.map((q) => {
            return {
        ...
        {#snippet item(value, i)}
            <div class="flex-1 row-gap">
                ...
                {#if value.dynamic}
                    <DropdownButton
                        value={value.value}
                        onChange={(v) => onDynamicValueChange(i, v)}
                ...
                {:else}
                    <input
                        ...
                        value={value.value}
                        class="flex-1"
                        onchange={(e) => onNodeValueChange(i, e)}
                    />
                {/if}
                <IconButton

<script lang="ts">
    ...
    import EditChecklistConditionCard from "@perfice/components/sharedWidgets/checklist/EditChecklistConditionCard.svelte";
    import { faPlus } from "@fortawesome/free-solid-svg-icons";
    import IconButton from "@perfice/components/base/button/IconButton.svelte";
    import EditChecklistConditionModal from "@perfice/components/sharedWidgets/checklist/EditChecklistConditionModal.svelte";
    import {
    ...
    function onDeleteOption(condition: ChecklistCondition) {
        onChange({
            ...settings,
            conditions: deleteIdentifiedInArray(
                settings.conditions,
                condition.id,
            ),
        });
...
<div class="row-between mt-2">
    <span class="text-gray-500 font-bold dark:text-white">Time scope</span>
    <BindableDropdownButton
        value={settings.timeScope}
        items={SIMPLE_TIME_SCOPE_TYPES}
        onChange={onTimeScopeChange}
    />
</div>
<div class="row-gap mt-2">
    <h2 class="text-xl text-gray-500 font-bold dark:text-white">Items</h2>
    <IconButton icon={faPlus} onClick={addOption} />
</div>

<DragAndDropContainer
    zoneId="select-options"

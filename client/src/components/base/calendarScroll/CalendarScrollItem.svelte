<script lang="ts">
    ...
    let {
        ...
        selectedValue,
        onClick,
    }: { value: Date; selectedValue: Date; onClick: () => void } = $props();

    let selected = $derived(selectedValue.getTime() == value.getTime());
</script>
...
<button
	...
	dark:bg-inherit bg-white {selected
        ? 'md:border-green-500 text-green-500  dark:text-green-200'
        : 'md:border dark:md:border-gray-500'}"
>

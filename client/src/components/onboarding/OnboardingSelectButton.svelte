<script lang="ts">
    ...
    function onClick() {
        ...
        if (selected) {
            updateSelectState(
                $state.snapshot(selectState).filter((s) => s.item != item.name),
            );
        } else {
            updateSelectState([
                ...$state.snapshot(selectState),
                { category: category, item: item.name, default: false },
            ]);
        }
    }
    ...
    let selected = $derived(
        selectState?.some(
            (s) => s.category == category && s.item == item.name,
        ) ?? false,
    );
    ...
    let buttonClass = $derived(
        selected
            ? `bg-white border-green-500 dark:bg-gray-700`
...
<button
    ...
    class="{buttonClass} hover-feedback border-2 rounded-xl py-2 px-4 flex flex-col gap-2 justify-center items-center"
>
    {#if item.icon}

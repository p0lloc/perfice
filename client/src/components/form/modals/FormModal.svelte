<script lang="ts">
    ...
    export function open(
        ...
        displayFormat: TextOrDynamic[],
        logDate: Date,
        availableTemplates: FormTemplate[],
        existingAnswers?: Record<string, PrimitiveValue>,
        entry?: JournalEntry,
    ) {
        form = logForm;
        date = logDate;
    ...
    function createOrUpdateTemplate(
        ...
        if (editingTemplate != null) {
            forms.updateFormTemplate(
                $state.snapshot(editingTemplate),
                name,
                extractedValues,
            );
        } else {
    ...
    function confirm() {
        ...
        if (currentTemplateName != null) {
            createOrUpdateTemplate(
                $state.snapshot(currentTemplateName),
                answers,
            );
        }
...
>
    ...
    {#if currentTemplateName != null}
        <input
            ...
            bind:value={currentTemplateName}
            placeholder="Template name"
        />
        {#if editingTemplate == null}
            <p class="text-xs mt-2">
                A new template will be created with the provided answers.
            </p>
        {/if}
        <hr class="my-4" />
    {/if}
    ...
    <div class="overflow-y-scroll scrollbar-hide md:pb-0 pb-20">
        <FormEmbed bind:this={embed} {questions} {answers} />
    </div>

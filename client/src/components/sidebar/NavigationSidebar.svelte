<script lang="ts">
    ...
    function getMobileGridSize() {
        return SIDEBAR_LINKS.filter(
            (link) => link.showOnMobile !== false && link.bottom !== true,
        ).length;
    }
...
<div
    ...
    class="md:flex md:min-h-screen left-0 fixed bg-white dark:bg-gray-900 border-t dark-border md:border-0 md:bg-green-500
        dark:md:bg-green-600
        md:w-14 w-screen px-2 py-2 md:p-2 bottom-0 z-[150] flex flex-col justify-between"
>
    <div
        class="grid md:flex md:flex-col justify-between items-center gap-2"
        style:grid-template-columns="repeat({getMobileGridSize()}, minmax(0,
        1fr))"
    >
        {#each top as link}
            <SidebarButton

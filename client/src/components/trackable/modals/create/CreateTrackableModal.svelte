<script lang="ts">
    ...
    import {
        TRACKABLE_SUGGESTIONS,
        type TrackableSuggestion,
    } from "@perfice/model/trackable/suggestions";
    import CreateSingleValueTrackable from "@perfice/components/trackable/modals/create/CreateSingleValueTrackable.svelte";
    import type { FormQuestionDataType } from "@perfice/model/form/form";
    import {
        TRACKABLE_FORM_CATEGORY_DELIM,
        TRACKABLE_FORM_ENTITY_TYPE,
    ...
    let {
        ...
        onSelectSuggestion: (
            ...
            suggestion: TrackableSuggestion,
        ) => void;
        onSingleValue: (
            categoryId: string | null,
            name: string,
    ...
    <div class="md:max-h-96 overflow-y-scroll scrollbar-hide">
        ...
        {:else}
            ...
            <div class="row-gap text-2xl mt-8">
                ...
                <span class="text-gray-500 font-bold dark:text-white"
                    >Suggestions</span
                >
            </div>
            <div class="flex flex-col gap-2 mt-4">
                {#each TRACKABLE_SUGGESTIONS as group}
                    <p class="font-bold text-gray-500 text-xl dark:text-white">
                        {group.name}
                    </p>
                    <div
                        class="grid grid-cols-2 md:grid-cols-4 md:max-w-[75%] gap-2"
                    >
                        {#each group.suggestions as suggestion}
                            <button
                                onclick={() =>
                                    onSelected(categoryId, suggestion)}
                                class="flex flex-col justify-center rounded-xl items-center gap-2 min-h-28 border dark-border hover-feedback"
                            >
